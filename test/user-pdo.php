...
class UserPdo {
    ...
    {
        try {
            $this->connexion = new PDO("mysql:host=localhost;dbname=classes;charset=utf8", "root", "");
    ...
    public function register(string $login, string $email, string $password, string $firstname, string $lastname): array|string
    {
        ...
        try {
            $stmt->execute([$login, $email, $hash, $firstname, $lastname]);
        } catch (PDOException $e) {
    ...
    public function update(string $login, string $email, string $password, string $firstname, string $lastname): string
    {
        ...
        if ($this->id) {
            ...
            try {
                $stmt->execute([$login, $email, $hash, $firstname, $lastname, $this->id]);
            } catch (PDOException $e) {
                if ($e->errorInfo[1] === 1062) {
                    return "Cet email existe déjà.";
                }
...
<body>
   <nav class="navbar">
    ...
    <div class="nav-links">
      <ul>
        <li class="active"><a href="User.php">mysqli</a></li>
      </ul>
    </div>
  </nav>
